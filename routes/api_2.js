...
var guild = "分會";
db2.collection('building').find().sort('rank').limit(1).toArray(function(err, result) {
	cycle = result[0].cycle;
	cycle_len = result[0].cycle_len;
	lowerBound = result[0].lowerBound;
});
var schedule_special;
db2.collection('special').find().toArray(function(err, result) {
	schedule_special = result;
});
var time_forbidden;
db2.collection('time').find().toArray(function(err, result) {
...
router.get('/building', function(req, res, next) {
  db2.collection('building').find().sort('rank').toArray(function(err, result) {
    if (err) throw err;
...
router.post('/building', function(req, res, next) {
  ...
  db2.collection('building').insert({name: name,limit: limit,rank: rank}, function(e, result){
		if (e) return next(e);
...
router.put('/building', function(req, res, next) {
  ...
  var limit = req.body.limit === 'null'? '' : parseInt(req.body.limit) ;
  var rank = req.body.rank === 'null'? '' : parseInt(req.body.rank) ;
  db2.collection('building').update({_id:  mongoskin.helper.toObjectID(req.body._id) },{name: name,limit: limit,rank: rank}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	schedule();
  });
...
router.delete('/building', function(req, res, next) {
  db2.collection('building').remove({_id: mongoskin.helper.toObjectID(req.body._id)}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	schedule();
  });
...
router.put('/schedule', function(req, res, next) {
  ...
  db2.collection('building').update({_id:  mongoskin.helper.toObjectID(req.body._id) },{cycle: cycle_s,cycle_len: cycle_len_s,lowerBound:lowerBound_s}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	cycle = cycle_s;
...
function schedule()
{
	...
	if(check)
	{
		db2.collection('building').find().sort('rank').toArray(function(err, building_data) {
			...
			if (err) throw err;
			db.collection('userdata').find({'type':guild}).toArray(algo);
...
router.get('/deploy', function(req, res, next) {
	  db2.collection('deploy').find().sort('rank').toArray(function(err, result) {
		if (err) throw err;
...
router.post('/special', function(req, res, next) {
  ...
  db2.collection('special').insert({name: name,power: power}, function(e, result){
		if (e) return next(e);
		res.send((result===1)?{msg:'success'}:{msg:'error'});
		db2.collection('special').find().toArray(function(err, result) {
		schedule_special = result;
...
router.put('/special', function(req, res, next) {
  ...
  var name = req.body.name === 'null'? '' : req.body.name ;
  var power = req.body.power === 'null'? '' : parseInt(req.body.power) ;
  db2.collection('special').update({_id:  mongoskin.helper.toObjectID(req.body._id) },{name: name,power: power}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	db2.collection('special').find().toArray(function(err, result) {
	schedule_special = result;
...
router.delete('/special', function(req, res, next) {
  db2.collection('special').remove({_id: mongoskin.helper.toObjectID(req.body._id)}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	db2.collection('special').find().toArray(function(err, result) {
	schedule_special = result;
...
router.post('/time', function(req, res, next) {
  ...
  db2.collection('time').insert({day: day,after: after,before: before}, function(e, result){
		if (e) return next(e);
		res.send((result===1)?{msg:'success'}:{msg:'error'});
		db2.collection('time').find().toArray(function(err, result) {
			time_forbidden = result;
...
router.put('/time', function(req, res, next) {
  ...
  var after = req.body.after === 'null'? '' : parseInt(req.body.after);
  var before = req.body.before === 'null'? '' : parseInt(req.body.before);
  db2.collection('time').update({_id:  mongoskin.helper.toObjectID(req.body._id) },{day: day,after: after,before: before}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	db2.collection('time').find().toArray(function(err, result) {
		time_forbidden = result;
...
router.delete('/time', function(req, res, next) {
  db2.collection('time').remove({_id: mongoskin.helper.toObjectID(req.body._id)}, function(e, result){
    if (e) return next(e);
    res.send((result===1)?{msg:'success'}:{msg:'error'});
	db2.collection('time').find().toArray(function(err, result) {
		time_forbidden = result;
